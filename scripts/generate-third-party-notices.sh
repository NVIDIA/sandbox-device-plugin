...
# Exactly what 'make build' compiles into nvidia-sandbox-device-plugin.
PACKAGES=("./cmd/...")
...
check_prerequisites() {
    ...
    # Probe by running it, not with -x: bin/ is not tracked, so a binary left
    # behind by a build for another platform passes -x and then cannot exec.
    ...
    # CGO on, matching the released build: 'make build' runs in an image that
    # has gcc, so the build constraints go-licenses evaluates must match.
...
prepare_workspace() {
    ...
    local t="${TMPDIR:-/tmp}/sandbox-device-plugin-notices"
...
collect_licenses() {
    ...
    for platform in "${PLATFORMS[@]}"; do
        ...
        # golang.org/x/*, google.golang.org/*, go.yaml.in/* and gopkg.in/*.
...
# 'go-licenses save' preserves source permissions, so restore write access or
# the next platform's copy into the shared tree fails.
merge_licenses() {
    cp -R "$1/." "$2/"
    chmod -R u+w "$2"
}
...
# license, so keeping one would hide a module's second license behind its first.
collapse_index() {
...
# this repo at HEAD, which stops describing released content once
# main moves.
...
build_index() {
    ...
    # empty field renders as "Unknown" through the fallback in the table, so both
    # have to fail here or an entry that attributes nothing would ship.
    ...
    if cut -d, -f4 "${INDEX_FILE}" | LC_ALL=C grep -qx 'unknown'; then
        die "some entries could not be matched to a module in ${MODULES_TXT}." \
            "Run 'go mod vendor' so modules.txt covers everything under vendor/."
    fi
}
...
This file lists the third-party Go modules that NVIDIA Sandbox Device Plugin
redistributes, along with the verbatim text of each module's license. In
particular, this covers the Go modules statically linked into the
`nvidia-sandbox-device-plugin` command built from `cmd/`, resolved as the union
across every released image platform. That command is the only binary this
project builds into the `nvidia-sandbox-device-plugin` image; the
`gpu-feature-discovery` binary in that image is copied from the
`nvcr.io/nvidia/k8s-device-plugin` image and is not covered here. Go standard
library packages are excluded; they are covered by the license of the Go
distribution itself.

The `nvidia-sandbox-device-plugin` image uses `nvcr.io/nvidia/distroless/go` as
a base image. All of the OSS packages and source included in this image can be
found at <https://developer.nvidia.com/w/distroless-oss/index.html>. A
statically compiled busybox binary is added to the image, which is licensed
under GPLv2.
